...
<title>{title} - CyberGuard</title>{cls.STYLE}</head>
<body><div class="container">
<div class="header"><h1>🛡️ {title}</h1>
...
<div class="footer">CyberGuard Professional Security Toolkit v{VERSION} | Report generated {ts}</div>
    ...
    def assessment_report(cls, findings: List[dict], scores: dict,
        ...
        for cat, sc in scores.items():
            ...
            body += f'<div class="label">{cat} (Grade {sc.get("grade", "N/A")})</div></div>'
        body += '</div>'
        ...
        if findings:
            ...
            for f in findings:
                ...
                body += f'<div class="card {sev}">'
                body += f'<h3>{cls._severity_badge(f.get("severity", "LOW"))} {f.get("title", "")}</h3>'
                body += f'<p>{f.get("description", "")}</p>'
                if f.get("recommendation"):
                    body += f'<div class="recommendation"><strong>Recommendation:</strong> {f["recommendation"]}</div>'
                body += '</div>'
    ...
    def compliance_report(cls, results: List[dict], framework: str,
        ...
        for r in results:
            ...
            body += f'<tr><td>{r.get("id", "")}</td><td>{r.get("title", "")}</td>'
            body += f'<td>{badge}</td><td>{r.get("details", "")}</td></tr>'
        body += '</table>'
    ...
    def hardening_report(cls, categories: Dict[str, List[dict]], overall_score: dict) -> str:
        ...
        for cat_name, checks in categories.items():
            ...
            for c in checks:
                ...
                body += f'<tr><td>{c.get("title", "")}</td><td>{badge}</td><td>{c.get("details", "")}</td></tr>'
            body += '</table>'
    ...
    def vulnerability_report(cls, vulns: List[dict], score: dict) -> str:
        ...
        if vulns:
            ...
            for v in vulns:
                body += f'<tr><td>{v.get("id", "N/A")}</td>'
                body += f'<td>{cls._severity_badge(v.get("severity", "LOW"))}</td>'
                body += f'<td>{v.get("description", "")}</td>'
                body += f'<td>{v.get("affected", "")}</td></tr>'
            body += '</table>'
    ...
    def executive_summary(cls, grade: str, score: float, findings_count: int,
        ...
        if top_findings:
            ...
            for f in top_findings[:10]:
                sev = f.get("severity", "LOW").lower()
                body += f'<div class="card {sev}">'
                body += f'{cls._severity_badge(f.get("severity", "LOW"))} <strong>{f.get("title", "")}</strong>'
                body += f'<p>{f.get("description", "")}</p></div>'

        if recommendations:
            body += '<h2>Key Recommendations</h2>'
            for i, r in enumerate(recommendations[:10], 1):
                body += f'<div class="recommendation">{i}. {r}</div>'
...
class AlertManager:
    ...
    def configure_webhook(self, url: str, name: str = "default"):
    ...
    def _send_email(self, subject: str, message: str, cfg: dict):
        try:
            import smtplib
    ...
    def _send_webhook(self, subject: str, message: str, severity: str, url: str):
        try:
            payload = {
...
class CyberGuardToolkit:
    ...
    def _malware_analysis(self):
        ...
        if self.cmd.has_command("ldd"):
            rc, out, _ = self.cmd.run(["ldd", str(fp)], timeout=10)
            if rc == 0:
                UI.print_subsection("Shared Libraries")
                console.print(out[:2000])
